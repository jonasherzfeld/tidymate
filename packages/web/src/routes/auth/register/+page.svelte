<script lang="ts">
    ...
    let { data } = $props();
    const { form, errors, enhance } = superForm(data.register_form, {
        onSubmit: async () => {
            is_loading = true;
        },
...
<div>
    ...
    >
        ...
        <TextInput type="text" name="first_name" placeholder="First name" bind:value={$form.first_name}/>
        {#if $errors.first_name}<span class="invalid text-error">{$errors.first_name}</span>{/if}

        <TextInput type="text" name="last_name" placeholder="Last name" bind:value={$form.last_name}/>
        {#if $errors.last_name}<span class="invalid text-error">{$errors.last_name}</span>{/if}

        <TextInput type="password" name="password" placeholder="Password" bind:value={$form.password}>
            <path
                fill-rule="evenodd"
        ...
        <TextInput type="password" name="confirm_password" placeholder="Confirm Password" bind:value={$form.confirm_password}>
            <path
                fill-rule="evenodd"
                d="M14 6a4 4 0 0 1-4.899 3.899l-1.955 1.955a.5.5 0 0 1-.353.146H5v1.5a.5.5 0 0 1-.5.5h-2a.5.5 0 0 1-.5-.5v-2.293a.5.5 0 0 1 .146-.353l3.955-3.955A4 4 0 1 1 14 6Zm-4-2a.75.75 0 0 0 0 1.5.5.5 0 0 1 .5.5.75.75 0 0 0 1.5 0 2 2 0 0 0-2-2Z"
                clip-rule="evenodd"
        ...
        {#if $errors.confirm_password}<span class="invalid text-error">{$errors.confirm_password}</span>{/if}
