<script lang="ts">
    ...
    let is_active_join_id = $derived(data.house.join_id || action?.join_id);
    let creating_join_id = $state(false);

    let handleJoinId: TypedSubmitFunction<ActionData> = async () => {
    ...
    <div class="flex flex-col flex-1 gap-3 min-w-full">
        ...
        {#if data.user.is_admin}
            <div>
                ...
                <form
                    action="?/toggle_join_id"
                    method="POST"
                    use:enhance={handleJoinId}
                >
                    <div class="join">
                        <input
                            ...
                            placeholder="Join ID"
                            value={action?.success ? action?.join_id : data.house.join_id}
                        />
                        <button
