<script lang="ts">
    ...
    import { HEADER_MAPPING, type HeaderMap, type RestrictionType } from '$lib/utils/constants';
...
<div class="navbar p-0 pl-3 pr-3 min-h-0 flex justify-between">
    <div class="flex w-fit">
        <a href={is_logged_in && is_in_house ? '/home' : '/'} class="btn btn-ghost hover:bg-transparent text-xl p-0 mr-2">
            <Logo width="30px" /><span class="hidden lg:block">Tidymate</span></a
        >
        <div class="hidden lg:block">

        <ul class="menu menu-horizontal m-0 p-0">
            <MenuBlock position="header_left" restricted={menu_restriction} />
        </ul>
        </div>
    </div>
    ...
    <div class="flex-none gap-2">
        ...
        <div class="dropdown dropdown-end">
            ...
            >
                <MenuBlock position="avatar_dropdown" restricted={menu_restriction} />
            </ul>
        </div>
    </div>
</div>
