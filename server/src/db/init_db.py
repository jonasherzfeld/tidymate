...
def get_latest_migration_revision():
    ...
    try:
        ...
        current_app.logger.warning(
            f"Could not get latest migration revision: {e}")
        return None
...
def initialize_fresh_database():
    ...
    if latest_revision:
        current_app.logger.info(
            f"Marking migration {latest_revision} as applied...")

        # For fresh databases, skip Flask-Migrate stamp and create manually
        # This avoids potential circular dependency issues during
        # initialization
        current_app.logger.info(
            "Creating alembic_version table manually for fresh database...")

        try:
            # Use direct SQLite connection to avoid any SQLAlchemy session
            # issues
            import sqlite3
            ...
            if db_url.startswith('sqlite:///'):
                ...
                conn.close()
                current_app.logger.info(
                    f"Successfully created alembic_version table with version: {latest_revision}")
            else:
                ...
                with db.engine.connect() as conn:
                    ...
                    conn.execute(
                        text(f"INSERT INTO alembic_version (version_num) VALUES ('{latest_revision}')"))
                    conn.commit()
                current_app.logger.info(
                    f"Successfully created alembic_version table with version: {latest_revision}")
            ...
            current_app.logger.error(
                f"Failed to create alembic_version table: {e}")
            # Continue anyway - the app will still work, just without migration
            # tracking
            current_app.logger.warning(
                "Continuing without migration tracking...")
    else:
        current_app.logger.warning(
            "No migration files found. Database created without migration tracking.")
...
def upgrade_existing_database():
    ...
    try:
        ...
        if migration_thread.is_alive():
            ...
            current_app.logger.warning(
                " Migration is taking too long (>30s), trying alternative approach...")
        ...
        current_app.logger.error(
            f"Flask-Migrate upgrade failed or timed out: {e}")
        current_app.logger.warning(
            "Attempting direct SQL migration approach...")
        ...
        try:
            ...
            current_app.logger.info(
                f"Applying direct migration from {current_migration} to {latest_revision}")

            # Use SQLAlchemy to apply schema changes
            from sqlalchemy import text
            with db.engine.connect() as conn:
                # Apply pending schema changes generically
                success = apply_pending_schema_changes(
                    conn, current_migration, latest_revision)
                ...
                if success:
                    ...
                    conn.commit()
                    current_app.logger.info(
                        f"Updated alembic_version to: {latest_revision}")
                else:
                    raise Exception("Failed to apply schema changes")

        except Exception as direct_e:
            current_app.logger.error(
                f"Direct migration also failed: {direct_e}")
            current_app.logger.warning(
                "Attempting fallback: manual version update...")

            # Final fallback: Just update the version (assuming schema is
            # correct)
            try:
                latest_revision = get_latest_migration_revision()
                if latest_revision:
                    current_app.logger.info(
                        f"Manually updating alembic_version to latest: {latest_revision}")
                    ...
                    with db.engine.connect() as conn:
                        ...
                        conn.commit()
                    current_app.logger.info(
                        f"Manually updated alembic_version to: {latest_revision}")
                    current_app.logger.warning(
                        "Schema changes may not have been applied. Please verify your database schema.")
                else:
                    current_app.logger.error(
                        "Could not determine latest migration version")

            except Exception as manual_e:
                current_app.logger.error(
                    f"Manual migration update also failed: {manual_e}")
                current_app.logger.error(
                    "Application will continue, but database may be out of sync.")
                current_app.logger.error(
                    "Please run migrations manually: flask db upgrade")
...
def initialize_database():
    ...
    if db_url.startswith('sqlite:///'):
        ...
        current_app.logger.info(
            "Non-SQLite database detected. Running migrations...")
    ...
    if not db_exists:
        ...
        current_app.logger.info(
            f"Existing database detected. Current version: {current_migration}, Latest: {latest_migration}")

        if current_migration is None:
            if alembic_table_exists:
                # alembic_version table exists but is empty - this is the edge
                # case
                current_app.logger.warning(
                    "Database has empty alembic_version table. Populating with latest migration...")

                if latest_migration:
                    try:
                        # Insert the latest migration directly into the
                        # existing empty table
                        import sqlite3
                        conn = sqlite3.connect(db_path)
                        cursor = conn.cursor()
                        cursor.execute(
                            f"INSERT INTO alembic_version (version_num) VALUES ('{latest_migration}')")
                        conn.commit()
                        conn.close()
                        current_app.logger.info(
                            f"Populated empty alembic_version table with: {latest_migration}")
                    except Exception as e:
                        current_app.logger.error(
                            f"Failed to populate alembic_version table: {e}")
                        # Fall back to stamping approach
                        try:
                            stamp(revision=latest_migration)
                            current_app.logger.info(
                                "Successfully stamped existing database with Flask-Migrate")
                        except Exception as stamp_e:
                            current_app.logger.error(
                                f"Stamping also failed: {stamp_e}")
                else:
                    current_app.logger.warning(
                        "No migrations found. Empty alembic_version table will remain empty.")
            else:
                # Database exists but no migration tracking at all
                current_app.logger.warning(
                    "Database exists but no migration version found.")

                if latest_migration:
                    current_app.logger.info(
                        f"Stamping database with latest migration: {latest_migration}")
                    ...
                    try:
                        ...
                        current_app.logger.warning(
                            f"Flask-Migrate stamp failed: {e}")
                        # Create alembic_version table manually
                        try:
                            db.session.execute(
                                text("CREATE TABLE IF NOT EXISTS alembic_version (version_num VARCHAR(32) NOT NULL)"))
                            db.session.execute(
                                text(f"INSERT INTO alembic_version (version_num) VALUES ('{latest_migration}')"))
                            db.session.commit()
                            current_app.logger.info(
                                f"Manually created alembic_version table and set version to {latest_migration}")
                        except Exception as manual_e:
                            current_app.logger.error(
                                f"Failed to manually create alembic_version: {manual_e}")
                            db.session.rollback()
                else:
                    current_app.logger.warning(
                        "No migrations found. Database will continue without migration tracking.")
        elif current_migration != latest_migration:
            # Normal migration scenario - migrations are pending
            current_app.logger.info(
                f"Migrations pending. Upgrading from {current_migration} to {latest_migration}")
            upgrade_existing_database()
        else:
...
def apply_pending_schema_changes(conn, current_version, target_version):
    ...
    try:
        ...
        for file_path in migrations_dir.glob("*.py"):
            if file_path.name != "__pycache__" and not file_path.name.startswith(
                    "__"):
        ...
        for migration_file in migration_files:
            ...
            current_app.logger.info(
                f"Applying schema changes from migration: {file_revision}")
            success = apply_migration_file_changes(conn, migration_file)

            if not success:
                current_app.logger.error(
                    f"Failed to apply changes from {file_revision}")
                return False
        ...
        current_app.logger.info(
            "Successfully applied all pending schema changes")
        return True
...
def apply_migration_file_changes(conn, migration_file):
    ...
    try:
        ...
        if 'add_column' in content.lower():
            current_app.logger.info(
                f"Found ADD COLUMN operation in {migration_file.name}")
            ...
            if 'item_serialized' in content:
                ...
                if 'item_serialized' not in columns:
                    ...
                    conn.execute(
                        text("ALTER TABLE history ADD COLUMN item_serialized TEXT"))
                    current_app.logger.info(
                        "Successfully added item_serialized column")
                else:
                    current_app.logger.info(
                        "item_serialized column already exists")

            # Look for other column additions by parsing the migration content
            # This can be extended for other column types and tables
            import re

            # Find add_column patterns like:
            # batch_op.add_column(sa.Column('column_name', sa.Type(), ...))
            ...
            for match in matches:
                ...
                if column_name != 'item_serialized':  # Already handled above
                    current_app.logger.info(
                        f"Found column to add: {column_name} ({column_type})")
                    # Add logic for other columns as needed

        # Pattern 2: DROP COLUMN operations (be careful with SQLite
        # limitations)
        if 'drop_column' in content.lower():
            current_app.logger.warning(
                f"Found DROP COLUMN operation in {migration_file.name}")
            current_app.logger.warning(
                "SQLite has limited DROP COLUMN support - skipping for safety")
            # SQLite doesn't support DROP COLUMN easily, so we skip this for
            # now

        # Pattern 3: CREATE TABLE operations
        if 'create_table' in content.lower():
            current_app.logger.info(
                f"Found CREATE TABLE operation in {migration_file.name}")
            # For CREATE TABLE, we rely on db.create_all() which should handle
            # this

        # Pattern 4: CREATE INDEX operations
        if 'create_index' in content.lower():
            current_app.logger.info(
                f"Found CREATE INDEX operation in {migration_file.name}")
        ...
        current_app.logger.error(
            f"Failed to apply migration file changes: {e}")
        conn.rollback()
        return False
