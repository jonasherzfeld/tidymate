...
<div
  class="pointer-events-none fixed inset-x-0 bottom-4 z-[100] flex flex-col items-center gap-2 px-4 sm:top-4 sm:right-4 sm:bottom-auto sm:left-auto sm:items-end">
  {#each toasts as t (t.id)}
    ...
    <div
      ...
      class={cn(
        "bg-base-100 border-neutral rounded-box pointer-events-auto relative flex w-full items-start gap-3 overflow-hidden border p-3 pl-4 shadow-[var(--shadow-lg)] sm:max-w-sm"
      )}>
      <span class={cn("absolute inset-y-0 left-0 w-1", style.accent)}></span>
      <div class={cn("flex h-8 w-8 shrink-0 items-center justify-center rounded-full", style.icon)}>
        <Icon class="h-4 w-4" />
      </div>
      ...
      <button
        class="text-base-content/50 hover:text-base-content hover:bg-base-200 rounded-field -m-1 inline-flex h-7 w-7 shrink-0 items-center justify-center transition-colors"
        aria-label="Dismiss"
