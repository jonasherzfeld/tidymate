...
<form class="m-2 mt-4 flex flex-col space-y-2" action="?/register" method="POST" use:enhance>
  ...
  <div>
    ...
    {#if $errors.last_name}<span class="invalid text-error ml-2 flex w-full text-start text-sm"
  ...
  {#if $form.is_join_home}
    <div>
      ...
      {#if $errors.join_id}<span class="invalid text-error ml-2 flex w-full text-start text-sm"
          >{$errors.join_id}</span
        >{/if}
    </div>
  {/if}
