...
export const GET: RequestHandler = async ({ cookies }) => {
  ...
  console.log(`[PUSH PROXY] GET vapid-public-key, session=${session ? "set" : "MISSING"}, backend=${BASE_API_URI}`);
  ...
  if (!res.ok) {
    ...
    console.error(`[PUSH PROXY] vapid-public-key failed: ${res.status} ${text}`);
    return json({ error: "Failed to fetch VAPID key" }, { status: res.status });
  }

  const data = await res.json();
  console.log(`[PUSH PROXY] vapid-public-key: key length=${data.public_key?.length}`);
  return json(data);
};
